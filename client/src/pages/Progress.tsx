...
interface ChartPoint {
  ...
  wpm: number; fillers: number;
  gaze: number; duration: number; passed: boolean;
}

export default function Progress() {
  const [sessions, setSessions] = useState<ProgressEntry[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get<ProgressEntry[]>('/api/progress')
      .then(r => { setSessions([...r.data].reverse()); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const chartData: ChartPoint[] = sessions.map((s, i) => ({
    idx: i + 1,
    label: `#${i + 1}`,
    wpm: s.metrics?.wpm ?? 0,
            ...
            <ChartCard title="Velocidad de habla (palabras/min)">
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="wpm" stroke="#06B6D4" strokeWidth={2}
                    dot={{ fill: '#06B6D4', r: 3 }} name="PPM" />
                </LineChart>
              </ResponsiveContainer>
            </ChartCard>
            ...
            <ChartCard title="Muletillas por minuto">
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData}>
                  ...
                  <YAxis tick={{ fill: '#94A3B8', fontSize: 10 }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="fillers" stroke="#8B5CF6" strokeWidth={2}
                    dot={{ fill: '#8B5CF6', r: 3 }} name="Muletillas/min" />
                </LineChart>
              </ResponsiveContainer>
            </ChartCard>
            ...
            <ChartCard title="Contacto visual (%)">
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="gaze" stroke="#10B981" strokeWidth={2}
                    dot={{ fill: '#10B981', r: 3 }} name="Contacto visual %" />
                </LineChart>
              </ResponsiveContainer>
            </ChartCard>
              ...
              {Array.from({ length: 10 }, (_, i) => {
                ...
                return (
                  <React.Fragment key={lvl}>
                    <div className={`flex flex-col items-center gap-1.5 ${isLocked ? 'opacity-40' : ''}`}
