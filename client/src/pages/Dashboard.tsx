...
function SessionCard({ session }: { session: ProgressEntry }) {
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        {session.xp_earned > 0 && (
          <div className="flex items-center gap-1 text-[#F59E0B]">
            <Zap className="w-3 h-3" /><span>+{session.xp_earned} XP</span>
          </div>
        )}
    ...
    const fetchData = async () => {
      try {
        const [progressRes, exRes] = await Promise.all([
          ...
          api.get<Exercise[]>(`/api/exercises?level=${u.current_level}`),
        ]);
        setSessions(progressRes.data.slice(0, 3));
        if (exRes.data.length > 0) setRecommended(exRes.data[0]);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-[#1F2937] border border-white/5 rounded-xl p-5" data-testid="level-card">
          ...
          <div className="flex items-end gap-3 mb-4">
            <span className="font-heading font-semibold text-5xl text-[#06B6D4]">{u.current_level}</span>
            <span className="text-[#94A3B8] text-sm mb-1">/ 10</span>
          </div>
          <XpBar xp={u.total_xp} level={u.current_level} />
        </div>
        ...
        <div className="bg-[#1F2937] border border-white/5 rounded-xl p-5" data-testid="xp-card">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-heading font-semibold text-3xl text-[#F1F5F9]">{u.total_xp}</span>
          </div>
